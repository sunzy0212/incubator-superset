...
func (m *Mysql) Query(chartType string, code common.Code) (interface{}, error) {
	...
	if rows, err := db.Execute(code.Code); err == nil {
		...
		case CHART_LINE, CHART_BAR, CHART_PIE:
			...
			for i := 0; i < rows.RowNumber(); i++ {
				...
				for _, index := range y_axis {
					var rs float64
					if rs, err = rows.GetFloat(i, index); err != nil {
						log.Warn(err)
						err = nil
					}
					ret.Datas[index] = append(ret.Datas[index], rs)
				}
			}
...
func (m *Mysql) getConn() (*client.Conn, error) {
	return client.Connect(fmt.Sprintf("%s:%s", m.Host, m.Port), m.Username, m.Password, m.Db)
}
...
func parseCols(sql string) map[string]string {
	...
	for _, v := range tmpFileds {
		...
		if len(alis) == 2 {
			fileds[strings.TrimSpace(alis[0])] = strings.TrimSpace(alis[1])
		} else {
