...
var (
	...
	ChartComponentTypes = []string{"TEXT", "CHART", "FLIPCHART"}
)
...
func (s *Service) PostCharts(env *rpcutil.Env) (chart common.Chart, err error) {
	...
	if flag := s.isChartComponentMatch(req.Type); !flag {
		err = ErrorPostChart(fmt.Errorf("chart component `type` is need, the type maybe `text|TEXT|chart|CHART|flipchart|FLIPCHART`"))
		return
	}
